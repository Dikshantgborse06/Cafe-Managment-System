...
var fs = require('fs'); //by default provided by node so no need to install any packages
var uuid= require('uuid');

var auth = require('../services/authentication');

//Creating API to generate the report
router.post('/generateReport', auth.authenticateToken,(req,res,next) =>{
    const generatedUuid = uuid.v1(); //v1 version, everytime it will generate unique one
    const orderDetails = req.body;
    var productDetailsReport = JSON.parse(orderDetails.productDetails); //is to remove the slash(vid-3:04:20)

    var query = "insert into bill(name,uuid,email,contactNumber,paymentMethod,total,productDetails,created) values(?,?,?,?,?,?,?,?)";
    ...
    //Whatever we are getting from frontend we're storing in productDetails not in our database
})
...
router.delete('/delete/:id',auth.authenticateToken,(req,res,next) =>{
    ...
    connection.query(query,[id],(err,results)=>{
        if(!err){
            if(results.affectedRows == 0){  // No deletion happened
